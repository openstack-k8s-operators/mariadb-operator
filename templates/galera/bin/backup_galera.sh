...
function finally {
    ...
    log "Backup of Galera cluster ${DB} finished (took ${timefmt})"
}
...
members=$(mysql -nN -u${dbuser} -p${dbpass} -h ${endpointfqdn} -e 'select node_name from mysql.wsrep_cluster_members;')
...
if [ "$target" = "" ]; then
    err "Could not take a backup of Galera cluster ${DB}"
else
...
wsrep_sst_rsync --role 'joiner' --address "${joinerfqdn}:4444" --datadir ${BACKUP_DIR} --defaults-file '' --defaults-group-suffix '' --parent $$ >${JOINER_LOG} 2>&1 &
joinerpid=$!
...
garbd -n backup -a gcomm://${targetfqdn}:4567 -g galera_cluster -l /dev/stdout --sst rsync:${joinerfqdn}:4444/rsync_sst  --donor ${target}
