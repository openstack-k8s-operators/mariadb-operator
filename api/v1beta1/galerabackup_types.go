...
type GaleraBackupSpec struct {
	...
	// Configuration for logical backup if used
	LogicalBackup *GaleraLogicalBackupSpec `json:"logicalBackup,omitempty"`
	// The schedule in Cron format
	// +kubebuilder:default="@hourly"
	Schedule string `json:"schedule"`
	// Temporarily suspend the backup schedule
	// +optional
	Suspend *bool `json:"suspend,omitempty"`
}

type GaleraLogicalBackupSpec struct {
	// Storage volume for backup data
	StorageClass string `json:"storageClass,omitempty"`
	// Storage Request for backup data
