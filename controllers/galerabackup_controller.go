...
func (r *GaleraBackupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	physicalPVC, logicalPVC := backup.BackupPVCs(instance, galera)

	err = r.Get(ctx, types.NamespacedName{Name: physicalPVC.Name, Namespace: physicalPVC.Namespace}, physicalPVC)
	if err != nil && k8s_errors.IsNotFound(err) {
		pvc.ObjectMeta = physicalPVC.ObjectMeta
		op, err := controllerutil.CreateOrPatch(ctx, helper.GetClient(), pvc, func() error {
			pvc.Spec = physicalPVC.Spec
		...
		helper.GetLogger().Info(fmt.Sprintf("Physical backup PVC %s - %s", pvc.Name, op))
	}

	if instance.Spec.LogicalBackup != nil {
		err = r.Get(ctx, types.NamespacedName{Name: logicalPVC.Name, Namespace: logicalPVC.Namespace}, logicalPVC)
		if err != nil && k8s_errors.IsNotFound(err) {
			pvc.ObjectMeta = logicalPVC.ObjectMeta
			op, err := controllerutil.CreateOrPatch(ctx, helper.GetClient(), pvc, func() error {
				pvc.Spec = logicalPVC.Spec
				err := controllerutil.SetControllerReference(helper.GetBeforeObject(), pvc, helper.GetScheme())
				if err != nil {
					return err
				}
			...
			helper.GetLogger().Info(fmt.Sprintf("Logical backup PVC %s - %s", pvc.Name, op))
		}
	}
...
func (r *GaleraBackupReconciler) reconcileDelete(ctx context.Context, instance *databasev1beta1.GaleraBackup, helper *helper.Helper) (ctrl.Result, error) {
