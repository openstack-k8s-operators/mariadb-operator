...
func (r *GaleraBackupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	backupPVC := mariadb.BackupPVC(instance, galera)
	...
	if err != nil && k8s_errors.IsNotFound(err) {
		...
		helper.GetLogger().Info(fmt.Sprintf("pvc %s - %s", pvc.Name, op))
	}

	preparedBackupCronJob := mariadb.BackupCronJob(instance, galera, hashOfHashes)
...
func (r *GaleraBackupReconciler) generateConfigMaps(
	...
	cfgScripts := fmt.Sprintf("%s-scripts", instance.Name)
	cms := []util.Template{
		// ScriptsConfigMap
		{
			Name:         cfgScripts,
